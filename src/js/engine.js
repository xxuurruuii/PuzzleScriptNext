...
Rule.prototype.queueCommands = function() {
	...
	for(var i=0;i<commands.length;i++) {
		...
		if (command[0] == 'log') {		// log is not queued
			...
			queuedCommand = `border,${border.direction},${border.amount}`;
...
function procInp(dir,dontDoWin,dontModify,bak,coord) {
	...
	if (dir < dirNames.length) {
		...
		const borderChanged = applyQueuedBorderCommands(curLevel.commandQueue, dontModify);
		if (dontModify && borderChanged) {
			if (verbose_logging) {
				consoleCacheDump();
			}
			return true;
