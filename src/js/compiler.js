    ...
    if (extraBoardEnabled) {
        ...
        function cloneLegendEntries(list) {
            ...
            for (const entry of list) {
                ...
                state.baseNameByExtraName[cloned[0]] = entry[0];
            }
            list.push(...clones);
        }
...
function concretizePropertyRule(state, rule, lineNumber) {
    const getBaseLegendName = name =>
        (state.baseNameByExtraName && state.baseNameByExtraName[name]) || name;
    const getTwinLegendName = name => {
        if (state.extraNameMap && state.extraNameMap[name]) {
            return state.extraNameMap[name];
        }
        if (state.baseNameByExtraName && state.baseNameByExtraName[name]) {
            return state.baseNameByExtraName[name];
        }
        return null;
    };
    const mapConcreteTypeForProperty = (targetProperty, concreteType) => {
        if (!(concreteType in state.objects)) {
            return concreteType;
        }
        const targetIsExtra = !!(state.baseNameByExtraName && state.baseNameByExtraName[targetProperty]);
        const concreteIsExtra = !!(state.baseNameByExtraName && state.baseNameByExtraName[concreteType]);
        if (targetIsExtra && !concreteIsExtra) {
            return (state.extraNameMap && state.extraNameMap[concreteType]) || concreteType;
        }
        if (!targetIsExtra && concreteIsExtra) {
            return (state.baseNameByExtraName && state.baseNameByExtraName[concreteType]) || concreteType;
        }
        return concreteType;
    };
    const concretizeEquivalentPropertiesInCell = (cell, property, concreteType) => {
        const names = [property];
        const twin = getTwinLegendName(property);
        if (twin && !names.includes(twin)) {
            names.push(twin);
        }
        for (const name of names) {
            const mappedConcrete = mapConcreteTypeForProperty(name, concreteType);
            concretizePropertyInCell(cell, name, mappedConcrete);
        }
    };
    ...
    const rawMappingProperties_l = [];
    const rawMappingProperties_r = [];
    ...
    for (let j = 0; j < rule.rhs.length; j++) {
        ...
        for (let k = 0; k < row_r.length; k++) {
            ...
            const canonical_l = properties_l.map(getBaseLegendName);
            ...
            for (let prop_n = 0; prop_n < properties_r.length; prop_n++) {
                ...
                if (!canonical_l.includes(getBaseLegendName(property))) {
                    ambiguousProperties[property] = true;
                }
            }
            properties_l.forEach(value => rawMappingProperties_l.push(value));
            properties_r.forEach(value => rawMappingProperties_r.push(value));
        }
    }
    ...
    function replaceMappedProperties(oldRule, numprops) {
        ...
        newRules.forEach((newRule,rindex) => {
            [ newRule.lhs, newRule.rhs ].forEach((side, sideIndex) => {
                const mappingSet = (sideIndex === 0) ? mappingPropertySetL : mappingPropertySetR;
                side.forEach(cellrow => {
                    cellrow.forEach(cell => {
                        for (let j = 0; j < cell.length; j += 2) {
                            if (cell[j] != "random"
                                && cell[j + 1] in state.propertiesDict
                                && mappingSet.has(cell[j + 1])) {
                    ...
                    for (var prop_n = 0; prop_n < properties.length; ++prop_n) {
                        ...
                        for (var l = 0; l < aliases.length; l++) {
                            ...
                            if (newrule.rhs.length > 0) {
                                concretizeEquivalentPropertiesInCell(newrule.rhs[j][k], property, concreteType); //do for the corresponding rhs cell as well
                            }
                ...
                if (occurrenceCount === 1) {
                    ...
                    for (var j = 0; j < cur_rule.rhs.length; j++) {
                        ...
                        for (var k = 0; k < cellRow_rhs.length; k++) {
                            ...
                            concretizeEquivalentPropertiesInCell(cell, property, concreteType);
                        }
                    }
                }
            }
